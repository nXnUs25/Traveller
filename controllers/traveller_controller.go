...
import (
	...
	mydomainv1alpha1 "github.com/nXnus25/Traveller/api/v1alpha1"
)
...
func (r *TravellerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	l := log.FromContext(ctx).WithValues("Traveller", req.NamespacedName)
	...
	if err := r.Get(context.TODO(), types.NamespacedName{Name: instance.Name, Namespace: instance.Namespace}, found); err != nil {
		str := fmt.Sprintf("err from instance Name [%v], Kind [%v], Namespace [%v]", instance.Name, instance.Kind, instance.Namespace)
		l.Error(err, str)
	}
	...
	l.Info("Skip reconcile: Deployment and service already exists",
